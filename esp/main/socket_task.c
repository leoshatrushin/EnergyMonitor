...
void socket_task(void *param) {
    ...
    while(1) {
        ...
        while (1) {
            int left_in_sector = MIN(partition_offset - socket_partition_offset, SECTOR_LEFT(socket_partition_offset));
            char server_offset_ack;
            if (Esp_tls_conn_write(tls, (const void *) &metadata, sizeof(metadata)) < 0) break;
            if (Esp_tls_conn_write(tls, socket_map_ptr, left_in_sector) < 0) break;
            if (esp_tls_conn_read(tls, &server_offset_ack, sizeof(char)) <= 0) break;
            metadata.lastValue = socket_map_ptr[socket_partition_offset + left_in_sector - 1];
            socket_partition_offset += left_in_sector;
        }
    }
}
